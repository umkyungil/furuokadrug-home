  ...
  const sendPaymentInfo = async (e) => {
    ...
    try {
      ...
      let aliPayQueryString = {
        ...
        'tn': telRef.current,
        'em': emailRef.current,
        'sucd': 'payment', // UPC 구시스템의 컨트롤 판넬에 등록한 redirect URL Code
        'lang': 'cn',
      ...
      // Query string작성
      let url = UPC_PAYMENT;
      Object.keys(aliPayQueryString).forEach(function(key, index) {
        url = url + (index === 0 ? "?" : "&") + key + "=" + aliPayQueryString[key];
      });

      // 로컬에서 하면 localhost에서 보내는 거라서 UPC에서 거절당한다
      // 테스트를 하려면 아래와 같이 DB에 직접 데이타를 보내서 해야 한다
      // ##########################TEST##########################
      // const paymentResult = axios.get(`${PAYMENT_SERVER}/alipay/register?rst=1&pid=1239&sod=${sod}&uniqueField=${uniqueFieldRef.current}`);
      // ##########################TEST##########################
...
export default AlipayConfirmPage;